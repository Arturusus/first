...
Временная сложность: O(n*log(n))
Почему:
Формирование начальной кучи занимает O(n) операций.
Удаление каждого элемента и восстановление структуры кучи требует O(logn) операций.
Всего потребуется n таких удалений, что даст общую сложность O(nlogn).
